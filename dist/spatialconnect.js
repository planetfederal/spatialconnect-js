...
/***/ function(module, exports, __webpack_require__) {
	...
	var SpatialConnect = (function () {
	  ...
	  connectWebViewJavascriptBridge(function (bridge) {
	    ...
	    bridge.registerHandler('lastKnownLocation', function (data) {
	      return sc.stream.lastKnownLocation.onNext(data);
	    });
	    bridge.registerHandler('spatialQuery', function (data) {
	      return sc.stream.spatialQuery.onNext(data);
	    });
	    bridge.registerHandler('storesList', function (data) {
	      return sc.stream.stores.onNext(data);
	    });
	  });

	  sc.action = {};
	  sc.action.disableGPS = function () {
	    return window.WebViewJavascriptBridge.send({ action: 'gps', value: 0 });
	  };
	  sc.action.enableGPS = function () {
	    return window.WebViewJavascriptBridge.send({ action: 'gps', value: 1 });
	  };
	  sc.action.stores = function () {
	    return window.WebViewJavascriptBridge.send({ action: 'stores' });
	  };
	  sc.action.spatialQuery = function (storeId) {
	    return window.WebViewJavascriptBridge.send({
	      action: 'spatialQuery',
	      value: storeId === undefined ? 'allstores' : storeId
	    });
	  };
	  sc.action.geospatialQuery = function (storeId) {
	    return window.WebViewJavascriptBridge.send({
	      action: 'geospatialQuery',
	      value: storeId === undefined ? 'allstores' : storeId
	    });
	  };
	  sc.action.sendMessage = function (action, value) {
	    return window.WebViewJavascriptBridge.send({
	      action: action,
	      value: value
	    });
	  };
	  sc.action.createFeature = function (featureObj) {
	    return window.WebViewJavascriptBridge.send({
	      action: 'createFeature',
	      value: { feature: featureObj }
	    });
	  };
	  sc.action.updateFeature = function (featureObj) {
	    return window.WebViewJavascriptBridge.send({
	      action: 'updateFeature',
	      value: featureObj
	    });
	  };
	  sc.action.deleteFeature = function (featureId) {
	    return window.WebViewJavascriptBridge.send({
